...
def openai_chat_json_request(test_name, payload):    
    print(f"\n===== [CHAT] {test_name} =====\n")
    try:
        headers = {
...
for i, payload in enumerate(invalid_payloads):
    openai_chat_json_request(f'invalid payload {i}', payload)
...
openai_chat_json_request("Normal Math (arithmetic):", {"messages":[
    ...
    { "role": "user", "content": "сколько будет 13243 * 5 - 6" }], 'r1_settings':{'math': True}, "model": model
}
)
